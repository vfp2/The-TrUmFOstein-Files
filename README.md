...
- [architecture/](architecture/) — the ongoing architectural discussion. Numbered entries; each one is open until superseded.
  ...
  - [05 — TypeDB decision and substrate schema](architecture/05-typedb-decision-and-schema.md) — closes the design phase. Commits to TypeDB 3.x (Rust-native) for the recursive-composition + n-ary-fidelity reasons; ships the [TypeQL schema](substrate/schema/schema.tql) and a working [ingestion pipeline](substrate/ingest/) that loads 161 documents / 4,174 pages / 9,654 claims / 22 incidents / 46 missions, including the D28 manifest-disagreement and Western US USPER1–7 multi-incident hyperedges. See [substrate/README.md](substrate/README.md) for run instructions and verification queries.
